"""
rdbms_backend_and.py
Real RDBMS backend for the AND (conjunction) scheme. Cloned from the
original main.py you gave me. Two changes only:
  - pinned to port 8002 so it can run alongside the doc-based AND backend
    (which stays on 8000) and the RDBMS-OR backend (8003)
  - added GET /rows, so the frontend can pull real table content once after
    upload and do its own local "expected result" comparison the same way
    the doc-based tabs already do (this reads the uploaded .db directly via
    stdlib sqlite3 - it does not touch rdbms_utils.inverted_index at all,
    so build_and_persist/TCVMap/normalise are completely untouched)
"""
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("rdbms_backend_and:app", host="0.0.0.0", port=8002, reload=True)